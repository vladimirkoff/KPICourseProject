...
const internal_get_a_class_named = (curr, searched_name) => {
  if (!curr) {
...
const internal_FixTheRanking = (rank) => {
  let ret_rank = 'NoRank';
  if (rank === 14) {
    ret_rank = 'ace';
  } else if (rank === 13) {
...
const internal_FixTheSuiting = (suit) => {
  if (suit === 'c') {
    suit = 'clubs';
  } else if (suit === 'd') {
    suit = 'diamonds';
...
const internal_GetCardImageUrl = (card) => {
...
const internal_setBackground = (diva, image, opacity) => {
  const komage = diva.style;
  komage.opacity = opacity;
  komage['background-image'] = image;
};

const internal_setCard = (diva, card, folded) => {
...
const internal_clickin_helper = (button, button_text, func_on_click) => {
  if (button_text === 0) {
...
const gui_hide_poker_table = () => {
  const table = document.getElementById('poker_table');
  table.style.visibility = 'hidden';
};

const gui_show_poker_table = () => {
  const table = document.getElementById('poker_table');
  table.style.visibility = 'visible';
};

const gui_set_player_name = (name, seat) => {
  const table = document.getElementById('poker_table');
...
const gui_hilite_player = (hilite_color, name_color, seat) => {
  const table = document.getElementById('poker_table');
  const current = 'seat' + seat;
  const seatloc = table.children[current];
  const chipsdiv = internal_get_a_class_named(seatloc, 'name-chips');
...
const gui_set_bankroll = (amount, seat) => {
  const table = document.getElementById('poker_table');
  const current = 'seat' + seat;
  const seatloc = table.children[current];
  const chipsdiv = internal_get_a_class_named(seatloc, 'name-chips');
...
const gui_set_player_cards = (card_a, card_b, seat, folded) => {
  const table = document.getElementById('poker_table');
  const current = 'seat' + seat;
  const seatloc = table.children[current];
...
const gui_lay_board_card = (n, the_card) => {
  let current = '';
  if (n === 0) {
    current = 'flop1';
  } else if (n === 1) {
...
const gui_burn_board_card = (n, the_card) => {
  let current = '';
...
const gui_write_basic_general = (pot_size) => {
  const table = document.getElementById('poker_table');
...
const gui_write_basic_general_text = (text) => {
  const table = document.getElementById('poker_table');
  const pot_div = table.children.pot;
  const total_div = pot_div.children['total-pot'];
...
const gui_log_to_history = (text_to_write) => {
...
const gui_hide_log_window = () => {
  const history = document.getElementById('history');
  history.style.display = 'none';
};

const gui_place_dealer_button = (seat) => {
...
const gui_hide_dealer_button = () => gui_place_dealer_button(-3);

const gui_hide_fold_call_click = () => {
...
const gui_setup_fold_call_click = (show_fold, call_text, fold_func, call_func, key_ev) => {
  const buttons = document.getElementById('action-options');
  const fold = buttons.children['fold-button'];
...
const curry_in_speedfunction = (speed_func) => {
...
const gui_set_selected_speed_option = (index) => {
  const buttons = document.getElementById('setup-options');
  const speed = buttons.children['speed-button'];
  const selector = speed.children['speed-selector'];
  const qqq = selector.children['speed-options'];
...
const internal_le_button = (buttons, button_name, button_func) => {
  const le_button = buttons.children[button_name];
  le_button.style.visibility = 'visible';
  le_button.onclick = button_func;
};

const gui_setup_option_buttons = (name_func, speed_func, help_func, check_func, mode_func) => {
  const buttons = document.getElementById('setup-options');
...
const internal_hide_le_button = (buttons, button_name) => {
  const le_button = buttons.children[button_name];
  le_button.style.visibility = 'hidden';
};

const gui_hide_setup_option_buttons = () => {
  const buttons = document.getElementById('setup-options');
...
const gui_hide_game_response = () => {
  const response = document.getElementById('game-response');
  response.style.visibility = 'hidden';
};

const gui_show_game_response = () => {
  const response = document.getElementById('game-response');
  response.style.visibility = 'visible';
};

const gui_write_game_response = (text) => {
  const response = document.getElementById('game-response');
  response.innerHTML = text;
};

const gui_set_game_response_font_color = (color) => {
  const response = document.getElementById('game-response');
  response.style.color = color;
};

const gui_write_guick_raise = (text) => {
...
const gui_hide_guick_raise = () => gui_write_guick_raise('');

const gui_write_modal_box = (text) => {
...
const gui_enable_shortcut_keys = (func) => {
  document.addEventListener('keydown', func);
}

const gui_disable_shortcut_keys = (func) => {
  document.removeEventListener('keydown', func);
};

const internal_get_theme_mode = () => {
...
const internal_set_theme_mode = (mode) => {
  setLocalStorage('currentmode', mode);
};

const gui_set_production_code_font_color = (color) => {
  const response = document.getElementById('production-code');
  response.style.color = color;
};

const internal_get_into_the_mode = (mode) => {
  const buttons = document.getElementById('setup-options');
...
const gui_get_theme_mode_highlite_color = () => {
  const mode = internal_get_theme_mode();
  let color = 'yellow';
  if (mode === 'light') {
    color = 'red';
