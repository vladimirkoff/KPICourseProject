...
let players;
let board; let deck_index; let button_index;
let current_bettor_index; let current_bet_amount; let current_min_raise;
...
function main() {
  ...
  if (players[current_bettor_index].status == 'BUST' ||
    ...
    if (current_bettor_index == 0) {
      ...
      const ret_function = function(key_event) {
      ...
      const do_fold = function() {
        actual_function(70, null);
      };
      const do_call = function() {
...
function write_player(n, hilite, show_cards) {
  ...
  let bet_text = 'TO BE OVERWRITTEN';
  const allin = 'Bet:';
