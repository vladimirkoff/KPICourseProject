...
function internal_get_hole_ranking() {
  ...
  let a = player.carda;
  let b = player.cardb;
  let n_rank_a = get_rank(a);
  let n_rank_b = get_rank(b);
...
function bot_get_postflop_bet() {
  ...
  const FULL_HOUSE = test_full_house(P);
  const FLUSH = test_flush(P);
  const STRAIGHT = test_straight(P);
  const THREE_OF_A_KIND = test_three_of_a_kind(P);
...
function internal_setup() {
  ...
  BANKROLL = P.bankroll;
  let total_bankrolls = get_pot_size();
  let number_of_players_in_game = 0;
  ...
  ID_CONF = 'MID';
  const avg_bankroll = total_bankrolls / number_of_players_in_game;
...
function internal_what_do_x(q, r) {
  ...
  while (1) {
    ...
    const probability = (q.substring(0, a) - 0) / 100;
    const action = q.substring(a + 1, b);
    q = q.substring(b + 1);
    p += probability;
    if (r <= p) {
